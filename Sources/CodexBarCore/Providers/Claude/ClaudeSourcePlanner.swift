...
public struct ClaudeFetchPlan: Equatable, Sendable {
    ...
    /// Auto mode keeps model-scoped usage available when it reaches the CLI fallback. The CLI
    /// omits limits such as Fable, while the web API exposes them. Reuse only an already discovered
