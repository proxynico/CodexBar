        ...
        bypassMergedMenuTrackingDeferral: Bool = false) -> Bool
    {
           ...
           let brand = ProviderBrandIcon.image(for: primaryProvider)
        {
            ...
            let signature = [
                ...
                "primary=\(IconRenderer.iconSignaturePercentBucket(primary))",
                "weekly=\(IconRenderer.iconSignaturePercentBucket(weekly))",
                "credits=\(IconRenderer.iconSignatureCreditsBucket(credits))",
                "stale=\(stale ? "1" : "0")",
