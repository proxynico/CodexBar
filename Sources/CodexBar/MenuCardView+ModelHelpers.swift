...
extension UsageMenuCardView.Model {
        ...
        percentStyle: PercentStyle) -> [Metric]
    {
        ...
        // Codex additional limits (e.g., Codex Spark) are too noisy for the main card in this fork.
        // Other providers' extra windows (Antigravity per-model quotas, Factory core windows, etc.)
        // are core data and must still render.
        if input.provider == .codex {
            return []
        }
        let visibleExtraRateWindows = if input.provider == .claude {
