...
- Claude CLI `auto` tries web, then CLI PTY; the app’s Claude `auto` path prefers OAuth, then web, then CLI. Ordinary
  app web failures can fall through to CLI, while cancellation remains terminal.
