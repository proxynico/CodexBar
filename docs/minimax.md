...
1) **Coding Plan API token**
   - Set in Preferences → Providers → MiniMax (stored in the [resolved config file](configuration.md)), `MINIMAX_CODING_API_KEY`,
     or `MINIMAX_API_KEY`.
...
4) **Manual session cookie header** (optional web-path override)
   - Stored in the resolved config file via Preferences → Providers → MiniMax (Cookie source → Manual).
